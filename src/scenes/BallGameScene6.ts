...
export default class BallGameScene6 extends Phaser.Scene{
    ...
    private rightRectInvent?: Phaser.Physics.Arcade.Image;
    ...
    create(){
        ...
        this.levelText = this.add.text(16, 16, 'level: 5', { fontSize: '32px', fill: '#000' });
        ...
        this.physics.add.collider(this.ballPlayer, this.rightRectInvent);
    ...
    private createInvent(){
        ...
        this.rightRectInvent = this.physics.add.image(100, 550, 'rightRect');
        this.rightRectInvent?.setInteractive();
        this.rightRectInvent?.setImmovable(true);
        this.rightRectInvent.body.allowGravity = false;
        ...
        for(let c = 0; c < this.squareInfo.count.col; c++){
            loop: for( let r = 0; r<this.squareInfo.count.row; r++){
                ...
                if(c === 4 && r === 0){
                    ...
                    let squareX = (c* this.squareInfo.width + this.squareInfo.offset.left);
                    let squareY = (r* this.squareInfo.height + this.squareInfo.offset.top);
                    this.rewOposCreat = this.physics.add.image(squareX, squareY, 'reqtOpos');
                    this.rewOposCreat.setImmovable(true);
                    this.rewOposCreat.body.allowGravity = false;
                } else {
                    let squareX = (c* this.squareInfo.width + this.squareInfo.offset.left);
    ...
    update () {
        ...
        if(this.newScene){
            this.scene.start("BallGameScene7");
        }
